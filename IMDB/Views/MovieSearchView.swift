...
struct MovieSearchView: View {
    ...
    var body: some View {
        ...
        .searchable(text: $movieSearchState.query, prompt: "Procurar filmes")
        .overlay(overlayView)
    ...
    private var overlayView: some View {
        ...
        case .empty:
            if movieSearchState.trimmedQuery.isEmpty {
                EmptyPlaceholderView(text: "Busque seus filmes favoritos", image: Image(systemName: "magnifyingglass"))
            } else {
                ProgressView()
            }
            
        case .success(let values) where values.isEmpty:
            EmptyPlaceholderView(text: "Sem Resultados", image: Image(systemName: "film"))
